...
const Tooltip = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <TooltipContext.Provider
      value={{ isOpen, setIsOpen, rect, setRect, anchorEl, setAnchorEl }}
    >
      {children}
    </TooltipContext.Provider>
  );
  ...
  (
    ...
    if (side === "top" || side === "bottom") {
      ...
      const left = clamp(
        ...
        Math.max(
          VIEWPORT_PADDING,
          viewportWidth - VIEWPORT_PADDING - measuredWidth,
        ),
      );
      ...
      style.top =
        side === "top"
          ? resolvedRect.top - sideOffset
          : resolvedRect.bottom + sideOffset;
      ...
      const top = clamp(
        ...
        Math.max(
          VIEWPORT_PADDING,
          viewportHeight - VIEWPORT_PADDING - measuredHeight,
        ),
      );
    ...
    const transforms = [
      baseTransform,
      animationOffset[side]?.[animationState] ||
        animationOffset.top[animationState],
    ].filter(Boolean);
    ...
    return createPortal(
      <div style={style}>
        <div
          ...
          className={cn(
            "whitespace-normal break-words text-left rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface-strong)]/90 backdrop-blur-md px-2.5 py-1.5 text-xs font-medium text-[var(--color-text-secondary)] shadow-lg leading-tight",
            className,
          )}
          style={{
            width: "max-content",
