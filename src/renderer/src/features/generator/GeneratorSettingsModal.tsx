...
import { X, Settings2, Hash, KeyRound, Zap, MonitorSmartphone, Shield, Globe } from 'lucide-react'
          ...
          >
            <div className="flex flex-col overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-2xl">
              ...
              <div className="p-4 space-y-4 max-h-[65vh] overflow-y-auto">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setConfig({ ...config, passwordLength: Number(e.target.value) })}
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-1.5">
                    <Button
                      variant="outline"
                      onClick={() => setConfig({ ...config, multiGenerateCount: Math.max(1, config.multiGenerateCount - 1) })}
                    ...
                    <Button
                      variant="outline"
                      onClick={() => setConfig({ ...config, multiGenerateCount: Math.min(50, config.multiGenerateCount + 1) })}
                      className="w-9 h-9 p-0 rounded-lg text-xs"
                    >+</Button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-2 pt-3 border-t border-[var(--color-border)]">
                  <label className="flex cursor-pointer items-center justify-between rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-strong)]/40 px-3 py-2.5 transition-colors hover:border-indigo-500/30">
                    ...
                    <CustomCheckbox 
                      ...
                      onChange={() => setConfig({ ...config, includeSpecialChars: !config.includeSpecialChars })} 
                    />
                  </label>
                  ...
                  <label className="flex cursor-pointer items-center justify-between rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-strong)]/40 px-3 py-2.5 transition-colors hover:border-indigo-500/30">
                    ...
                    <CustomCheckbox 
                      ...
                      onChange={() => setConfig({ ...config, autoSwapBrowser: !config.autoSwapBrowser })} 
                    />
                  </label>
                </div>
              </div>
              ...
              <div className="border-t border-[var(--color-border)] bg-[var(--color-surface-strong)]/20 px-4 py-3 flex items-center justify-end gap-2">
                <Button variant="ghost" onClick={onClose} className="h-8 text-xs px-4">
                  Cancel
                </Button>
                <Button 
                  variant="default" 
                  onClick={() => { handleUpdateConfig(); onClose() }} 
