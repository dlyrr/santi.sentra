    ...
    queryFn: async () => {
      ...
      if (pBadgesData && pBadgesData.data) {
        ...
        const badgeIds = Array.from(
          new Set(badges.map((b: any) => b.id).filter((id: any) => !!id)),
        );
...
export function useUserOutfits(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const allOutfits = [
        ...(editableResponse.data || []).map((o: any) => ({
          ...o,
          type: "Creation",
        })),
        ...(purchasedResponse.data || []).map((o: any) => ({
          ...o,
          type: "Purchased",
        })),
      ];
...
export function useUserFriendsStatuses(
  ...
  return useQuery({
    queryKey: [
      ...queryKeys.userProfile.friends(userId, cookie),
      "statuses",
      friendIds,
    ] as const,
    queryFn: async () => {
