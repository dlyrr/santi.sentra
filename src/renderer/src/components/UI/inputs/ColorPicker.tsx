...
const Slider = ({
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    try {
      (
        node as Element & { setPointerCapture?: (id: number) => void }
    ...
    const handlePointerUp = () => {
      ...
      try {
        (
          node as Element & { releasePointerCapture?: (id: number) => void }
        ).releasePointerCapture?.((e as any).pointerId);
      } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={
          isVertical ? { top: `${percentage}%` } : { left: `${percentage}%` }
        }
      />
    </div>
  );
...
export const ColorPickerSelection = memo(
  ({ className, ...props }: ColorPickerSelectionProps) => {
    ...
    const calculatePositionFromColor = useCallback(
