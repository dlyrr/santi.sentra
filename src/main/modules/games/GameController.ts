...
export const registerGameHandlers = (): void => {
  ...
  handle(
    ...
    async () => {
      const getWindowsLogsPath = (): string => {
        ...
        console.log(
          "[GameController] LOCALAPPDATA has spaces:",
        ...
        console.log(
          "[GameController] USERPROFILE has spaces:",
        ...
        if (userProfile) {
          ...
          console.log(
            "[GameController] Trying fallback path (lowercase logs):",
            fallbackPath,
          );
          ...
          console.log(
            "[GameController] Trying fallback path (uppercase Logs):",
            fallbackPath,
          );
        ...
        console.error(
          "[GameController] Neither LOCALAPPDATA nor USERPROFILE are set!",
        );
        return "";
      };
      ...
      const getRecentPlacesFromLogs = async (
        ...
        if (!existsSync(LOGS_DIR)) {
          console.warn(
            "[GameController] Roblox logs directory NOT found at:",
            LOGS_DIR,
          );
          return emptyResult;
        }
        ...
        try {
          ...
          console.log(
            "[GameController] Successfully read directory. Found entries:",
            entries.length,
          );
        ...
        console.log(
          ...
          potentialLogFiles.length,
          "files",
        );

        if (potentialLogFiles.length === 0) {
          console.warn(
            "[GameController] No files found (excluding hidden). All files:",
            files,
          );
          return emptyResult;
        }
        ...
        const entries = await Promise.all(
          potentialLogFiles.map(async (file) => {
            ...
            try {
              ...
              console.error(
                `[GameController] Failed to read log ${file}:`,
                err,
              );
              return {
                lastModified: 0,
      ...
      if (universeIds.length > 0) {
        try {
          return await RobloxGameService.getGamesByUniverseIds(
            universeIds.map((id) => Number(id)),
          );
        } catch (err) {
          console.error(
            "[GameController] Failed to hydrate recent universeIds:",
            err,
          );
        }
      }
      ...
      if (placeIds.length > 0) {
        ...
        if (!cookie) {
          console.warn(
            "[GameController] Skipping placeId hydration for recent games: no cookie",
          );
          return [];
        }
        ...
        try {
          ...
          console.error(
            "[GameController] Failed to hydrate recent placeIds:",
            err,
          );
          return [];
        }
  ...
  handle(
    ...
    async (_, placeId, serverId) => {
      ...
      const cookie = accountWithCookie ? accountWithCookie.cookie : undefined;
      if (!cookie) throw new Error("No logged in account found");
      return RobloxGameService.getServerQueuePosition(
        placeId,
        serverId,
        cookie,
      );
