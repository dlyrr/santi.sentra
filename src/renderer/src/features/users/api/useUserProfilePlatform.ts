  ...
  return {
    ...
    robloxBadges: (robloxBadges?.robloxBadgeList ?? []).map((badge) => ({
      ...
      imageUrl: badge.type?.imageName
        ...
        : "",
      awardedAt: badge.createdTime?.seconds
        ? new Date(badge.createdTime.seconds * 1000)
        : undefined,
    })),
...
export function useUserProfilePlatform(
  userId: number,
  cookie: string,
...
export function useUserProfileHeader(
  userId: number,
  cookie: string,
  enabled: boolean = true,
) {
...
export function useUserProfileAbout(
  userId: number,
  cookie: string,
  enabled: boolean = true,
) {
...
export function useUserProfileCollections(
  userId: number,
  cookie: string,
  enabled: boolean = true,
) {
