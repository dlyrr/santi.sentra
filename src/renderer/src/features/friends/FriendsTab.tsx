...
const FriendsTab = ({
    ...
    queries: targetAccounts.map((acc) => ({
      ...
      queryFn: async (): Promise<Friend[]> => {
        ...
        const fetchedFriends = await window.api.getFriends(
          acc.cookie,
          acc.userId ? parseInt(acc.userId) : undefined,
        );
        return fetchedFriends.map((f: any) => ({
          id: f.id,
          accountId: acc.id,
          displayName:
            f.displayName || f.username || `User ${f.userId || f.id}`,
  ...
  const getSectionKey = useCallback(
    (friend: Friend): SectionKey => {
      ...
      if (
        friend.status === AccountStatus.InGame &&
  ...
  const sections: {
    ...
    {
      key: AccountStatus.InStudio,
      label: "In Studio",
      icon: Wrench,
      color: "text-orange-500",
    },
    {
      key: AccountStatus.Online,
      label: "Online",
      icon: Wifi,
      color: "text-blue-500",
    },
    {
      key: AccountStatus.Offline,
      label: "Offline",
      icon: WifiOff,
  ...
  return (
    <TooltipProvider>
      <div className="flex flex-col h-full bg-[var(--color-app-bg)]">
          ...
          <div className="flex items-center gap-3">
            ...
            {targetAccounts.length === 1 && (
              <Button
                variant="default"
                onClick={() => setIsAddFriendModalOpen(true)}
                className="gap-2 shrink-0 bg-[var(--color-surface-hover)] hover:bg-[var(--color-surface-hover-2)] text-[var(--color-text-primary)] border border-[var(--color-border)]"
              >
                <UserPlus size={16} />
            ...
            {targetAccounts.length === 1 && (
              <Button
                ...
                className="gap-2 shrink-0 relative bg-[var(--color-surface-hover)] hover:bg-[var(--color-surface-hover-2)] text-[var(--color-text-primary)] border border-[var(--color-border)]"
              >
                <Users size={16} />
                <span>Requests</span>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  disabled={
                    isLoading || isFetching || targetAccounts.length === 0
                  }
                >
                  <RefreshCw
                    size={18}
        ...
        {}
        <div
          className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-[var(--color-app-bg)] relative"
          ref={friendsListRef}
        >
              ...
              {sections.map(({ key, label, icon: Icon, color }) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`flex items-center gap-2 text-xs font-bold uppercase tracking-[0.15em] ${color}`}
                      >
                        {Icon && <Icon size={14} />}
                        {label}
                      </div>
                          ...
                          return (
                            <div key={friend.id}>
                              <Card
                                ...
                                className="relative flex items-center p-4 bg-[var(--color-surface)]/60 border-[var(--color-border-subtle)] hover:border-[var(--color-border)] hover:bg-[var(--color-surface-hover)]/60 hover:shadow-lg hover:shadow-black/30 transition-all duration-200 group cursor-pointer hover:-translate-y-0.5 h-[88px] overflow-hidden"
                              >
                                {}
                                {(friend.status === AccountStatus.InGame ||
                                  friend.status === AccountStatus.Online) && (
                                  <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/5 to-transparent pointer-events-none" />
                                )}

                                <div className="relative mr-4 shrink-0">
                                  <Avatar className="w-14 h-14 ring-2 ring-[var(--color-border-subtle)] group-hover:ring-[var(--color-border)] transition-all">
                                    <AvatarImage
                                  ...
                                  {isFavorite && (
                                    <div className="absolute -top-1.5 -right-1.5 bg-yellow-500 rounded-full p-1 z-10 shadow-md shadow-yellow-900/30">
                                      <Star
                                        size={9}
                                        className="fill-yellow-900 text-yellow-900"
                                      />
                                    </div>
                                  )}
                                </div>
        ...
        <FriendContextMenu
          ...
          isFavorite={
            activeContextMenu
              ? favorites.includes(activeContextMenu.userId.toString())
              : false
          }
