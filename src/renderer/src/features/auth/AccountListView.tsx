...
const AccountListView = ({
  ...
  const itemContent = useCallback(
    (_index: number, account: Account) => {
      const isSelected =
        selectedIds instanceof Set ? selectedIds.has(account.id) : false;
              ...
              <div className="h-8 w-8 flex-shrink-0 relative">
                <img
                  className={[
                    ...
                    isSelected
                      ? "ring-[var(--accent-color-border)] shadow-[0_0_8px_var(--accent-color-faint)]"
              ...
              <div>
                ...
                >
                  ...
                  {account.age !== undefined && (
                    <span
                      className="flex items-center gap-1 bg-[var(--color-surface-muted)] px-1.5 py-0.5 rounded text-[9px] font-medium border border-[var(--color-border)] text-[var(--color-text-muted)] shrink-0 ml-1"
                      title={`${account.age} years old`}
                    >
                      <Clock size={9} />
                      {account.age}y
                    </span>
  ...
  const tableComponents = useMemo<TableComponents<Account, AccountRowContext>>(
    () => ({
      ...
      TableHead: forwardRef<HTMLTableSectionElement>((props, ref) => (
        <thead
          ref={ref}
          className="sticky top-0 z-10 bg-[var(--color-surface)] shadow-[0_1px_0_var(--color-border)]"
          {...props}
        />
      )),
      ...
      TableRow: ({ item: account, ...props }) => {
        ...
        const isSelected =
          ctx.selectedIds instanceof Set
            ? ctx.selectedIds.has(account.id)
            : false;
        return (
          <tr
            {...props}
