...
export function useBulkInventory(
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<BulkInventoryItem[]> => {
      ...
      try {
        const thumbResponse = await window.api.getBatchThumbnails(uniqueAssetIds)
    ...
    enabled: accounts.length >= 2 && assetTypeIds.length > 0 && (options?.enabled ?? true),
