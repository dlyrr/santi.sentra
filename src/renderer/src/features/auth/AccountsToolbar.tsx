    ...
    <div className="shrink-0 h-[64px] bg-[var(--color-surface)] border-b border-[var(--color-border)] flex items-center justify-between px-6 gap-4 z-20">
      ...
      <div className="flex items-center gap-2 shrink-0 flex-1 justify-end min-w-0">
          ...
          <div className="flex items-center gap-1.5 mr-2 shrink-0">
            {isValidating ? (
              <div className="flex flex-col gap-1 shrink-0">
                <div className="flex items-center gap-2 h-9 px-3 rounded-lg border border-emerald-500/40 bg-emerald-500/10 backdrop-blur-md min-w-[200px] relative overflow-hidden shadow-[inset_0_0_12px_rgba(16,185,129,0.1)]">
                  ...
                  <div className="relative z-10 flex items-center gap-2 w-full justify-between">
                    <Loader2
                      size={13}
                      className={`animate-spin shrink-0 ${validationStatus === "ratelimited" ? "text-amber-400" : "text-emerald-400"}`}
                    />
                    <span
                      className={`text-xs font-semibold font-mono truncate ${validationStatus === "ratelimited" ? "text-amber-400" : "text-emerald-400"}`}
                    >
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={onValidateAccountsClick}
                className="gap-2 shrink-0 border-emerald-500/50 text-emerald-400 hover:bg-emerald-500/10 hover:shadow-[0_0_12px_rgba(16,185,129,0.15)] transition-shadow duration-200"
              >
                <Wifi size={16} />
        ...
        <Button
          ...
          className="gap-2 shrink-0 mr-1 shadow-sm hover:shadow-[0_0_12px_var(--accent-color-ring)] transition-shadow duration-200"
        >
          <UserPlus size={16} />
        ...
        <CustomDropdown
          ...
          value={statusFilter}
          onChange={(value) =>
            onStatusFilterChange(value as AccountStatus | "All")
          }
          placeholder="Filter"
