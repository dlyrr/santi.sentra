...
interface FriendsToolbarProps {
  ...
  onStatusFilterChange: (status: 'All' | 'Favorites' | 'Online' | 'InGame') => void
  onAddFriend: () => void
  ...
  return (
    <div className="shrink-0 h-[64px] bg-[var(--color-surface)] border-b border-[var(--color-border)] flex items-center justify-between px-6 gap-4 z-20">
      <div className="flex items-center gap-4 shrink-0">
        <h1 className="text-lg font-bold text-[var(--color-text-primary)] leading-none tracking-tight">Friends</h1>
      ...
      <div className="flex items-center gap-2 shrink-0 flex-1 justify-end min-w-0">
        <Button variant="default" onClick={onAddFriend} className="gap-2 h-9 px-3 shrink-0">
          <UserPlus size={16} />
          <span className="text-sm font-semibold">Add Friend</span>
        </Button>
        <Button variant="secondary" onClick={onFriendRequests} className="gap-2 h-9 px-3 shrink-0">
          <Users size={16} />
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" size="icon" onClick={onRefresh} disabled={isLoading || isFetching} className="h-9 w-9 shrink-0">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" size="icon" onClick={onViewModeToggle} className="h-9 w-9 shrink-0">
              {viewMode === 'list' ? <Grid size={17} /> : <List size={17} />}
            </Button>
          </TooltipTrigger>
          <TooltipContent>{viewMode === 'list' ? 'Switch to Grid View' : 'Switch to List View'}</TooltipContent>
        </Tooltip>
      </div>
    </div>
  )
