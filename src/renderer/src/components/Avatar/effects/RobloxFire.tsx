...
const loadDDSTexture = (
  ...
  return new Promise((resolve) => {
    ...
    loader.load(
      ...
      (error) => {
        console.warn(
          `[RobloxFire] Failed to load ${name}, using fallback:`,
          error,
        );
        resolve(fallbackFn());
      },
...
export const RobloxFire: React.FC<RobloxFireProps> = ({
  ...
  const secondColor = useMemo(() => {
    return secondaryColor instanceof THREE.Color
      ? secondaryColor
  ...
  const [sparksTexture, setSparksTexture] = useState<THREE.Texture | null>(
    null,
  );
  const [sparksColorTexture, setSparksColorTexture] =
    useState<THREE.Texture | null>(null);
  const [sparksAlphaTexture, setSparksAlphaTexture] =
    useState<THREE.Texture | null>(null);
  ...
  useEffect(() => {
    ...
    loadDDSTexture(
      fireSparksMainDdsUrl,
      "fire_sparks_main.dds",
      createFallbackFireTexture,
    ).then(setSparksTexture);
    loadDDSTexture(
      fireSparksColorDdsUrl,
      "fire_sparks_color.dds",
      createFallbackColorTexture,
    ).then(setSparksColorTexture);
    loadDDSTexture(
      commonAlphaDdsUrl,
      "common_alpha.dds",
      createFallbackAlphaTexture,
    ).then(setSparksAlphaTexture);
  }, []);
  ...
  const sparksMaterial = useMemo(() => {
    if (!sparksTexture || !sparksColorTexture || !sparksAlphaTexture)
      return null;
    return new THREE.ShaderMaterial({
      uniforms: {
  ...
  useFrame((_, delta) => {
    ...
    if (pointsRef.current) {
      ...
      const robloxSize = clampedSize / 3.5;
      const localForceY =
        0.5 * ((robloxSize * robloxSize) / 4 + 0.7 * clampedHeat);
    ...
    if (sparksPointsRef.current) {
      ...
      const sparksPositions = sparksGeometry.attributes.position
