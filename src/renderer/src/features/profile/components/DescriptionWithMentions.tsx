...
export const DescriptionWithMentions: React.FC<
  DescriptionWithMentionsProps
> = ({ description, mentionSourceText, onSelectProfile }) => {
  const { mentionProfiles, ensureMentionProfile } =
    useMentionProfiles(mentionSourceText);
  ...
  const descriptionContent = useMemo(() => {
    ...
    while ((match = regex.exec(description)) !== null) {
      if (match.index > lastIndex) {
        ...
        nodes.push(
          <React.Fragment key={`desc-text-${partIndex++}`}>
            {linkifiedSegment}
          </React.Fragment>,
        );
      }
