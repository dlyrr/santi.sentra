...
export const registerStorageHandlers = (): void => {
  ...
  handle(
    "set-accounts-view-mode",
    z.tuple([z.enum(["list", "grid"])]),
    async (_, mode) => {
  ...
  handle(
    "save-accounts",
    z.tuple([z.array(accountSchema)]),
    async (_, accounts) => {
      console.log(
        "[StorageController] save-accounts handler called with",
        accounts.length,
        "accounts",
      );
      ...
      console.log(
        "[StorageController] save-accounts: PIN hash exists:",
        !!pinHash,
        "PIN currently verified:",
        isPinVerified,
      );
      ...
      try {
        ...
        console.error(
          "[StorageController] save-accounts handler FAILED:",
  ...
  handle(
    "set-exclude-full-games",
    z.tuple([z.boolean()]),
    async (_, excludeFullGames) => {
        ...
        UserAgentService.startAutoSwap(intervalMinutes || 30, (ua) => {
          ...
          BrowserWindow.getAllWindows().forEach((window) => {
            try {
              ...
              console.error(
                "[StorageController] Failed to set UA on window during auto-swap:",
                e,
              );
            }
          });
