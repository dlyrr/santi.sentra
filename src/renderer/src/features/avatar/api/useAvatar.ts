...
export function useUserOutfits(account: Account | null, isEditable: boolean) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<InventoryItem[]> => {
      ...
      const response = await window.api.getUserOutfits(
        cookie,
        userId,
        isEditable,
        1,
      ...
      let thumbMap = new Map<number, string>();
      try {
        const thumbResponse = await window.api.getBatchThumbnails(
          outfitIds,
          "Outfit",
        );
        thumbMap = new Map(
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      if (userId) {
        ...
        void refreshAccountAvatarHeadshot(
          queryClient,
          accountId,
          userId,
          "420x420",
...
export function useUpdateOutfit(account: Account | null) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.avatar.outfits(accountId, true),
      });
      queryClient.invalidateQueries({
        queryKey: queryKeys.avatar.outfits(accountId, false),
      });
    },
  });
}
