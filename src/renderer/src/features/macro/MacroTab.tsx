...
import {
  ...
  CardHeader,
  CardTitle,
} from "@renderer/components/UI/display/Card";
...
const MacroCard = React.memo(
  ({ macro, isBuiltIn, onPlay, onDelete }: MacroCardProps) => (
    <div
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          ...
          {macro.description && (
            <p className="text-xs text-[var(--color-text-muted)] dark:text-[var(--color-text-muted)] leading-relaxed">
              {macro.description}
            </p>
          )}
...
export const MacroTab = () => {
  ...
  const handlePlayMacro = useCallback(
  ...
  const { builtInMacros, customMacros } = useMemo(() => {
    ...
    built.sort((a, b) => {
      ...
      return (
        (categoryOrder[catA as keyof typeof categoryOrder] ?? 3) -
        (categoryOrder[catB as keyof typeof categoryOrder] ?? 3)
      );
    });
            ...
            {isRecording && (
              <div className="bg-blue-50 /20 p-4 rounded-lg border border-blue-200 ">
                ...
                <p className="text-xs text-blue-600  font-mono">
                  {recordingProgress.eventCount} events •{" "}
                  {(recordingProgress.duration / 1000).toFixed(1)}s
                </p>
              </div>
            )}
          ...
          <CardContent>
            {customMacros.length === 0 ? (
              <p className="text-sm text-[var(--color-text-muted)] dark:text-[var(--color-text-muted)]">
                No custom macros recorded yet. Use the Recording section to
                create one.
              </p>
            ) : (
