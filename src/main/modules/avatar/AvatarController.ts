...
export const registerAvatarHandlers = (): void => {
  ...
  handle(
    ...
    async (_, cookieRaw, userId, assetTypeId, cursor) => {
      const cookie = RobloxAuthService.extractCookie(cookieRaw);
      return RobloxAvatarService.getInventory(
        cookie,
        userId,
        assetTypeId,
        cursor,
  ...
  handle(
    ...
    async (_, targetIds, type) => {
      return RobloxAvatarService.getBatchThumbnails(
        targetIds,
        undefined,
        undefined,
        type,
  ...
  handle(
    ...
    z.tuple([
      z.string(),
      z.number(),
      outfitDetailsSchema.partial().passthrough(),
    ]),
  ...
  handle(
    ...
    z.tuple([
      z.string(),
      z.array(z.number()),
      z.enum(["Asset", "Bundle"]).optional(),
    ]),
    async (_, cookieRaw, assetIds, itemType) => {
      const cookie = RobloxAuthService.extractCookie(cookieRaw);
      return RobloxAvatarService.getBatchAssetDetails(
        cookie,
        assetIds,
        itemType || "Asset",
      );
  ...
  handle(
    ...
    async (_, collectibleItemId, limit, cursor) => {
      return RobloxAvatarService.getAssetResellers(
        collectibleItemId,
        limit || 100,
        cursor,
      );
  ...
  handle(
    ...
    async (_, cookieRaw, userIdRaw, assetId, itemType) => {
      ...
      return RobloxInventoryService.checkAssetOwnership(
        cookie,
        userId,
        assetId,
        itemType,
  ...
  handle("get-rolimons-player", z.tuple([z.number()]), async (_, userId) => {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch Rolimons player data: ${response.status}`,
      );
    }
  ...
  handle(
    ...
    async (_, keyword, limit, creatorName) => {
      return RobloxAvatarService.searchCatalog(
        keyword,
        limit || 30,
        creatorName,
      );
  ...
  handle(
    ...
    async (event, assetId, type, assetName) => {
      ...
      if (!thumbResponse.ok)
        throw new Error(
          `Failed to fetch 3D thumbnail data: ${thumbResponse.status}`,
        );
