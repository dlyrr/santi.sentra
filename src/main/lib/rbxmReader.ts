  ...
  static convertInterleaved<T>(
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let j = 0; j < stride; j++)
        buf[stride - 1 - j] = bytes[i + j * count];
      res[i] = fn(buf);
    }
    return res;
  }
...
const Parsers: Partial<Record<DataType, TypeParser>> = {
  [DataType.String]: {
    read(r, count, out) {
      for (let i = 0; i < count; i++)
        out.push({ type: DataType.String, value: r.getString() });
    },
  },
  [DataType.Bool]: {
    read(r, count, out) {
      for (let i = 0; i < count; i++)
        out.push({ type: DataType.Bool, value: r.getBool() });
    },
  },
  [DataType.Int32]: {
    read(r, count, out) {
  ...
  [DataType.Float64]: {
    read(r, count, out) {
      for (let i = 0; i < count; i++)
        out.push({ type: DataType.Float64, value: r.getFloat64() });
    },
  },
  [DataType.UDim]: {
    read(r, count, out) {
  ...
  [DataType.UDim2]: {
    read(r, count, out) {
      ...
      for (let i = 0; i < count; i++)
        out.push({
          type: DataType.UDim2,
          value: new UDim2Value(
            new UDimValue(sx[i], ox[i]),
            new UDimValue(sy[i], oy[i]),
          ),
        });
    },
  },
    ...
    read(r, count, out) {
      ...
      for (let i = 0; i < count; i++)
        out.push({
          ...
          value: new Vector3Value(x[i], y[i], z[i]),
        });
    },
  },
  [DataType.Vector2]: {
    read(r, count, out) {
      ...
      for (let i = 0; i < count; i++)
        out.push({
          ...
          value: new Vector2Value(x[i], y[i]),
        });
    },
  },
  [DataType.Color3]: {
    read(r, count, out) {
      ...
      for (let i = 0; i < count; i++)
        out.push({
          ...
          value: new Color3Value(red[i], green[i], blue[i]),
        });
    },
  },
  ...
  [DataType.Enum]: {
    read(r, count, out) {
      ...
      for (let i = 0; i < count; i++)
        out.push({
          type: DataType.Enum,
          value: EnumItemValue.MakeUnknownEnum(vals[i]),
        });
    },
  },
  [DataType.Referent]: {
  ...
  [DataType.BrickColor]: {
    read(r, count, out) {
      const vals = r.getInterleavedInt32(count);
      for (let i = 0; i < count; i++)
        out.push({ type: DataType.BrickColor, value: vals[i] });
    },
  },
