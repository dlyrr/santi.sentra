...
interface InstallationsActions {
  ...
  updateInstallation: (
    id: string,
    updates: Partial<RobloxInstallation>,
  ) => void;
...
export const useSelectedInstallationId = () =>
  useInstallationsStore((state) => state.selectedId);
export const useDeployHistory = () =>
  useInstallationsStore((state) => state.deployHistory);
