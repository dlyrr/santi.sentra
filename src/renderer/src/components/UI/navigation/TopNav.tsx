...
import { Search, Menu, LogOut, ArrowRightLeft, Ticket, Heart, ChevronDown } from 'lucide-react'
import { Account, TabId } from '@renderer/types'
import { Tooltip, TooltipTrigger, TooltipContent, TooltipProvider } from '../display/Tooltip'
...
import { getVisibleSidebarTabs, sanitizeSidebarHidden, sanitizeSidebarOrder } from '@shared/navigation'
...
const TopNavItem = ({ icon: Icon, label, isActive, onClick }: TopNavItemProps) => (
  <Tooltip>
...
const TopNav = ({
  ...
  const accountForProfile =
    selectedAccounts.length > 0 ? selectedAccounts[0] : selectedAccount || primaryAccount
      ...
      >
        ...
        >
          {navTabs.map((tab, index) => {
            const previous = navTabs[index - 1] as SidebarTabDefinition | undefined
        ...
        >
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-surface-hover)] transition-all"
            title="Search (Ctrl+K)"
          >
            <Search size={16} />
          </button>
