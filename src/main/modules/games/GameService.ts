...
export class RobloxGameService {
  ...
  static async getGameIconThumbnail(
    ...
    try {
      ...
      if (
        thumbResult.data &&
        thumbResult.data.length > 0 &&
        thumbResult.data[0].imageUrl
      ) {
  ...
  static async getGameSorts(sessionId: string = randomUUID()) {
    ...
    const gameSorts = rawSorts.filter(
      (s) => s.contentType === "Games" || !s.contentType,
    );
  ...
  static async searchGames(query: string, sessionId: string = randomUUID()) {
    ...
    const universeIds = [
      ...new Set(validGames.map((g: any) => g.universeId)),
    ] as number[];
  ...
  static async getRecentlyPlayedGames(
    ...
    if (!cookie) {
      console.warn(
        "[RobloxGameService] No cookie found for recently played games",
      );
      return [];
    }
  ...
  private static async hydrateGames(
    universeIds: number[],
    initialData: unknown[] = [],
  ) {
    if (universeIds.length === 0) return [];
      ...
      for (const ids of chunks) {
        try {
          const detailsResult = await request(
            z.object({ data: z.array(gameDetailsSchema) }),
            {
    ...
    try {
      ...
      for (const ids of chunks) {
        try {
          const thumbResult = await request(
            z.object({ data: z.array(gameThumbnailSchema) }),
            {
    ...
    if (initialData.length === 0) {
      try {
        ...
        for (const ids of chunks) {
          try {
            const votesResult = await request(
              z.object({ data: z.array(gameVoteSchema) }),
              {
    ...
    const dedupedData = (initialData as any[]).filter((g: any) => {
      ...
      if (seenInResult.has(uid)) return false;
      seenInResult.add(uid);
      return true;
    });

    return dedupedData.map((g: any) => {
      const d = detailsMap[g.universeId];
      const thumb = thumbnailsMap[g.universeId];

      const ageRating =
        d?.genre === "All" || d?.isAllGenre ? "All Ages" : "Not rated";
  ...
  static async getGamesByPlaceIds(placeIds: string[], cookie?: string) {
    ...
    for (const placeId of placeIds) {
      ...
      if (typeof universeId !== "number" || seenUniverseIds.has(universeId))
        continue;
  ...
  static async getUniverseIdFromPlaceId(
    placeId: number,
    cookie?: string,
  ): Promise<number | null> {
    try {
  ...
  static async getGameServers(
    ...
    try {
      ...
      if (error.statusCode === 404) {
        console.warn(
          `getGameServers returned 404 for ${placeId}, trying with Universe ID...`,
        );
        const universeId = await this.getUniverseIdFromPlaceId(
          Number(placeId),
          cookie,
        );
  ...
  static async getServerRegion(
    ...
    try {
      ...
      if (joinResult.status === 10 || joinResult.status === 6) {
        console.warn(
          "[RobloxGameService] Server full or restricted, cannot get IP directly.",
        );
        return "Full/Restricted";
      }
      if (joinResult.status === 22) {
        console.warn(
          "[RobloxGameService] Server is queued, cannot get IP directly.",
        );
        return "Queued";
      }
