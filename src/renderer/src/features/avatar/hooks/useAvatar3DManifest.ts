...
export type Avatar3DManifestResponse = z.infer<
  typeof avatar3DManifestResponseSchema
>;

export const ASSET_TYPES_WITH_3D_MODELS = new Set([
  8, 41, 42, 43, 44, 45, 46, 47, 19, 17, 27, 29, 28, 30, 31, 67, 70, 71, 72, 4,
  40, 10, 79,
]);
...
export const avatar3DKeys = {
  ...
  manifest: (userId: string | number) =>
    [...avatar3DKeys.all, "manifest", String(userId)] as const,
  assetManifest: (assetId: string | number) =>
    [...avatar3DKeys.all, "assetManifest", String(assetId)] as const,
};

export const useAvatar3DManifest = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!cookie)
        throw new Error(
          "cookie is required for authenticated 3D manifest request",
        );
...
export const useAsset3DManifest = (
  ...
  const supports3D =
    assetTypeId == null || ASSET_TYPES_WITH_3D_MODELS.has(assetTypeId);
  ...
  return useQuery({
    ...
    retry: (failureCount, error) => {
      ...
      if (
        message.includes("3D view not available") ||
