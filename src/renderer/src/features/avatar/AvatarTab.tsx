...
const AvatarTab: React.FC<AvatarTabProps> = ({ account }) => {
  ...
  const effectiveAccount =
    account || (selectedAccounts.length > 0 ? selectedAccounts[0] : null);
  ...
  const { data: currentAvatarData, refetch: refetchCurrentAvatar } =
    useCurrentAvatar(effectiveAccount);
  ...
  const {
    ...
    isLoading: isLoadingSingleInventory,
  } = useInventory(effectiveAccount, assetTypeIds, {
    enabled: isInventoryCat && assetTypeIds.length > 0 && !isBulkMode,
  });

  const { data: bulkInventoryData = [], isLoading: isLoadingBulkInventory } =
    useBulkInventory(selectedAccounts, assetTypeIds, {
      enabled: isInventoryCat && assetTypeIds.length > 0 && isBulkMode,
    });

  const inventoryData = isBulkMode ? bulkInventoryData : singleInventoryData;
  const isLoadingInventory = isBulkMode
    ? isLoadingBulkInventory
    : isLoadingSingleInventory;

  const isEditable = subCategory === "Creations";
  const { data: outfitsData = [], isLoading: isLoadingOutfits } =
    useUserOutfits(effectiveAccount, isEditable);
  ...
  const handleContextMenu = (e: React.MouseEvent, item: InventoryItem) => {
    e.preventDefault();
    const isCreation =
      mainCategory === "Characters" && subCategory === "Creations";
    setContextMenu({
      id: item.id,
      name: item.name,
  ...
  const handleDeleteOutfitWithConfirmation = (
    outfitId: number,
    name: string,
  ) => {
        ...
        {selectedIds.size >= 2 && (
          <button
            ...
            onClick={async () => {
              if (
                !confirm(
                  "Are you sure you want to reset all selected avatars to their default appearance?",
                )
              )
                return;
              ...
              try {
                ...
                showNotification(
                  `Reset ${selectedAccounts.length} avatars to default successfully`,
                  "success",
                );
              } catch (err) {
                console.error(err);
        ...
        <div className="flex h-[calc(100%-80px)] flex-col gap-6 lg:flex-row">
          <div className="w-full lg:w-1/2 h-full">
            <AvatarViewport
              ...
              account={effectiveAccount}
              currentAvatarType={
                currentAvatarType === "R6" || currentAvatarType === "R15"
                  ? currentAvatarType
                  : null
              }
            ...
            <div className="flex flex-col border-b border-[var(--color-border)] bg-[var(--color-app-bg)] z-10 shadow-sm">
              ...
              <SearchBar
                ...
                show={
                  !(
                    mainCategory === "Body" &&
                    (subCategory === "Skin" || subCategory === "Scale")
                  )
                }
              />
            </div>
