  ...
  const handleSave = async () => {
    ...
    if (selectedPart === "all") {
      ...
      if (currentBodyColors) {
        BODY_PARTS.forEach((part) => {
          if (part.id !== "all") {
            const existing = getColorFromBodyColors(currentBodyColors, part.id);
            if (existing) {
              bodyColors[part.id] = existing.startsWith("#")
                ? existing
                : `#${existing}`;
            } else {
              bodyColors[part.id] = "#FFFFFF";
            }
          }
        });
      }
    ...
    <div className="flex flex-col gap-6 p-6 h-full w-full max-w-4xl mx-auto overflow-y-auto">
      ...
      <div className="flex justify-center pt-4 pb-8">
        <Button
          ...
          size="lg"
        >
          {setBodyColorsMutation.isPending
            ? "Updating..."
            : "Update Skin Color"}
        </Button>
      </div>
