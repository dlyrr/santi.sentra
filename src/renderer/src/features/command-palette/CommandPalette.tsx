...
const UNSELECTED_BG =
  "color-mix(in srgb, var(--color-text-primary) 0%, transparent)";
const PANEL_BACKDROP =
  "color-mix(in srgb, var(--color-app-bg) 82%, transparent)";
const STRIP_BG = "var(--color-surface)";
const TILE_BG =
  "color-mix(in srgb, var(--color-surface-muted) 90%, transparent)";
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {result.isRare && (
              <Gem size={11} className="text-purple-400 flex-shrink-0" />
            )}
            {result.isHyped && (
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-[var(--color-text-muted)] mt-0.5">
            {result.value !== null ? (
              <span className="text-emerald-400/90">
                {result.value.toLocaleString()} Value
              </span>
            ) : (
              <span className="text-[var(--color-text-muted)]">
                {result.rap.toLocaleString()} RAP
              </span>
            )}
            <span className="text-[var(--color-text-muted)]">·</span>
            <span className={DEMAND_COLORS[result.demand]}>
              D: {result.demandLabel}
            </span>
            <span className="text-[var(--color-text-muted)]">·</span>
...
const UniversalPlayerRow = memo(
  ({
    ...
    onSelect,
    onHover,
  }: UniversalPlayerRowProps) => {
    const isSelected = idx === selectedIndex;
    const badge = result.isFriend
      ? resultTypeBadges.friend
      : resultTypeBadges.player;
    return (
      <motion.button
        initial={{ opacity: 0, y: 6 }}
        animate={{ opacity: 1, y: 0 }}
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  const universalSearchResults = useMemo<UniversalSearchResult[]>(() => {
    ...
    catalogResults.forEach((item) => {
      const isLimitedDuplicate = limitedsResults.some(
        (l) => l.id === item.AssetId,
      );
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (step === "search") {
        if (query.startsWith(">")) {
          ...
          const filteredCmds = cmdQuery
            ? commands.filter((cmd) => {
                ...
                const matchDesc = cmd.description
                  ?.toLowerCase()
                  .includes(cmdQuery);
          ...
          if (e.key === "ArrowDown") {
            e.preventDefault();
            setSelectedIndex(
              Math.min(selectedIndex + 1, universalSearchResults.length - 1),
            );
          } else if (e.key === "ArrowUp") {
            e.preventDefault();
            setSelectedIndex(Math.max(selectedIndex - 1, 0));
            ...
            e.key === "Enter" &&
            universalSearchResults[selectedIndex]
          ) {
            e.preventDefault();
        ...
        if (e.key === "ArrowDown" && filteredFriends.length > 0) {
          e.preventDefault();
          setSuggestionIndex(
            Math.min(suggestionIndex + 1, filteredFriends.length - 1),
          );
        ...
        if (e.key === "ArrowDown") {
          ...
          setResultSelectedIndex(
            Math.min(resultSelectedIndex + 1, searchResults.length - 1),
          );
        } else if (e.key === "ArrowUp") {
          e.preventDefault();
                    ...
                    <div className="py-1.5">
                      <div className="px-4 py-2 text-[10px] font-semibold uppercase tracking-wider flex items-center gap-2 text-[var(--color-text-secondary)]">
                        ...
                        <span className="ml-auto text-[var(--color-text-muted)] normal-case font-normal tracking-normal">
                          {filteredCmds.length}{" "}
                          {filteredCmds.length === 1 ? "result" : "results"}
                        </span>
                      </div>
