  ...
  useEffect(() => {
    if (currentScales) {
      if (typeof currentScales.height === "number")
        setHeight(currentScales.height);
      if (typeof currentScales.width === "number")
        setWidth(currentScales.width);
    ...
    <div className="flex flex-col gap-6 p-6 h-full w-full max-w-2xl mx-auto overflow-y-auto">
        ...
        <>
            ...
            <div className="flex gap-2 flex-wrap">
              {HEIGHT_OPTIONS.map((option) => (
                <Button
                  ...
                  className={cn(
                    "min-w-[60px]",
                    height === option && "ring-2 ring-emerald-500/50",
                  )}
                >
            ...
            <div className="flex gap-2 flex-wrap">
              {WIDTH_OPTIONS.map((option) => (
                <Button
                  ...
                  className={cn(
                    "min-w-[60px]",
                    width === option && "ring-2 ring-emerald-500/50",
                  )}
                >
                  {formatPercent(option)}
                </Button>
            ...
            <div className="flex gap-2 flex-wrap">
              {HEAD_OPTIONS.map((option) => (
                <Button
                  ...
                  className={cn(
                    "min-w-[60px]",
                    head === option && "ring-2 ring-emerald-500/50",
                  )}
                >
                  {formatPercent(option)}
                </Button>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm text-[var(--color-text-muted)]">
                {formatProportionPercent(bodyType)}
              </span>
            </div>
            <div className="relative">
              <input
      ...
      {avatarType === "R6" && (
        <div className="flex-1 flex items-center justify-center text-[var(--color-text-muted)] text-sm">
          <p>
            R6 avatars use classic body proportions and don&apos;t support
            custom scaling.
          </p>
        </div>
      )}
