      ...
      try {
        ...
        if (rolimonsData.value === null || rolimonsData.value === undefined) {
          callbacks.showNotification(
            `${user.displayName} has no value set on Rolimons`,
            "info",
          );
        } else {
          const formattedValue = formatNumber(rolimonsData.value);
          callbacks.showNotification(
            `${user.displayName}'s value: ${formattedValue} R$`,
            "success",
          );
        }
      } catch (e) {
        if (e instanceof Error) {
          if (e.message.includes("not found")) {
            ...
            callbacks.showNotification(
              `Failed to fetch value for "${username}"`,
              "error",
            );
          }
        } else {
    ...
    onInputSubmit: async (username) => {
      try {
        ...
        if (rolimonsData.rap === null || rolimonsData.rap === undefined) {
          callbacks.showNotification(
            `${user.displayName} has no RAP set on Rolimons`,
            "info",
          );
        } else {
          const formattedRap = formatNumber(rolimonsData.rap);
          callbacks.showNotification(
            `${user.displayName}'s RAP: ${formattedRap} R$`,
            "success",
          );
        }
      } catch (e) {
        if (e instanceof Error) {
          if (e.message.includes("not found")) {
            ...
            callbacks.showNotification(
              `Failed to fetch RAP for "${username}"`,
              "error",
            );
          }
        } else {
