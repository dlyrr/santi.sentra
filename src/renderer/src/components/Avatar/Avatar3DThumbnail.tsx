...
const AnimatedRig: React.FC<AnimatedRigProps> = ({
  children,
  isLoaded,
  objectType,
}) => {
...
export const Model3DViewer: React.FC<Model3DViewerProps> = ({
  ...
  const effectiveZoomLimits =
    zoomLimits ??
    (objectType === "asset" ? { min: 1, max: 15 } : { min: 4, max: 26 });
  ...
  const needsAlwaysFrameloop =
    isAutoRotating || hasFireEffects || hasSparklesEffects;
      ...
      >
        ...
        <Suspense fallback={null}>
          <AnimatedRig isLoaded={!isLoading} objectType={objectType}>
            ...
            {}
            {hasSparklesEffects && (
              <AssetSparklesEffects sparkles={sparkles} scale={1} />
            )}
          </AnimatedRig>
        </Suspense>
      </Canvas>
