...
const Object3DLoader: React.FC<Object3DLoaderProps> = ({
    ...
    const loadObject = async () => {
      try {
        ...
        if (group) {
          ...
          let cameraZ =
            Math.abs(maxDim / 2 / Math.tan(fov / 2)) * cameraDistanceFactor;
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const newRotationX =
        objectRef.current.rotation.x + deltaY * manualRotationSensitivity;
      objectRef.current.rotation.x = THREE.MathUtils.clamp(
        newRotationX,
        -Math.PI / 2,
        Math.PI / 2,
      );
...
const SceneLighting: React.FC<{ objectType: ObjectType }> = ({
  objectType,
}) => {
  return (
    <>
      <hemisphereLight
        args={[0xffffff, 0x080820, objectType === "asset" ? 1.2 : 1.1]}
      />
      <directionalLight
        position={[6, 16, 10]}
      ...
      <directionalLight
        position={[-6, 4, -12]}
        intensity={objectType === "asset" ? 0.7 : 0.6}
      />
      {objectType === "asset" && (
...
const Avatar3DCanvas: React.FC<Avatar3DCanvasProps> = ({
  ...
  const objectType: ObjectType | null =
    explicitType ||
    (manifestUrl ? "avatar" : userId ? "avatar" : assetId ? "asset" : null);
  ...
  const {
    ...
    !manifestUrl && objectType === "avatar" ? userId : undefined,
    cookie,
  );
  const {
    ...
    !manifestUrl && objectType === "asset" ? assetId : undefined,
    cookie,
    {
      assetTypeId,
    },
  ...
  const effectiveManifestUrl =
    manifestUrl || avatarManifestUrl || assetManifestUrl;
  const manifestError = avatarError || assetError;
  const manifestLoading = avatarLoading || assetLoading;

  const effectiveVerticalOffset =
    verticalOffset ?? (objectType === "avatar" ? -1.2 : 0);
  const effectiveZoomLimits =
    manualZoomLimits ??
    (objectType === "asset" ? { min: 1, max: 15 } : { min: 7, max: 26 });
  const effectiveCameraDistanceFactor =
    cameraDistanceFactor ?? (objectType === "asset" ? 2.2 : 1.8);

  useEffect(() => {
    if (manifestError) {
      const errorMessage =
        (manifestError as Error)?.message || "Failed to load 3D manifest";
        ...
        >
          ...
          <Suspense fallback={null}>
            <Object3DLoader
              ...
              objectName={
                objectId ? `${objectType}_${objectId}` : "render_preview"
              }
