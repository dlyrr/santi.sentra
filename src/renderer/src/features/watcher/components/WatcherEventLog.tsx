    ...
    <div className="flex-1 overflow-hidden flex flex-col border border-[var(--color-border)] rounded-[var(--control-radius)] bg-[var(--color-surface)]">
      ...
      <div className="flex-1 overflow-y-auto font-mono text-[11px] styled-scrollbar">
        {events.map((event, index) => {
          ...
          return (
            <div
              key={`${event.timestamp}-${event.type}-${event.sessionId}-${index}`}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span
                    className={`font-semibold uppercase tracking-wider text-[9px] px-1.5 py-0.5 rounded bg-[var(--color-surface-strong)] border border-[var(--color-border)] ${getEventColor(event.type)}`}
                  >
