...
export function useAssetRecommendationsQuery({
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const data = await (window as any).api.getAssetRecommendations(
        cookie || "",
        assetId,
      );
      ...
      if (!parsed.success) {
        console.warn(
          "[useAssetRecommendationsQuery] Validation warning:",
          parsed.error.issues,
        );
      }
  ...
  useEffect(() => {
    ...
    const recIds = query.data.data.filter(
