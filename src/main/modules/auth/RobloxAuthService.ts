...
import {
  quickLoginCodeSchema,
  quickLoginStatusSchema,
...
export class RobloxAuthService {
  ...
  static async getAuthenticationTicket(
    cookie: string,
    csrfToken: string,
  ): Promise<string> {
    ...
    try {
      ...
      if (
        ...
        if (newToken) {
          const updatedToken = Array.isArray(newToken)
            ? newToken[0]
            : (newToken as string);
    ...
    try {
      ...
      if (error instanceof RequestError) {
        if (
          ...
          (error.body === '"CodeInvalid"' ||
            ...
            (typeof error.body === "string" &&
              error.body.includes("CodeInvalid")))
        ) {
        ...
        if (error.statusCode === 403 && error.headers) {
          const token = error.headers["x-csrf-token"];
          if (token) {
            const csrfToken = Array.isArray(token)
              ? token[0]
              : (token as string);
  ...
  static async completeQuickLogin(
    code: string,
    privateKey: string,
  ): Promise<string> {
    ...
    try {
      await attemptLogin();
      throw new Error(
        "Expected 403 for CSRF token, but got success (unexpected)",
      );
    } catch (error) {
      if (
        error instanceof RequestError &&
        ...
        if (token) {
          ...
          if (setCookie) {
            ...
            const securityCookie = cookies.find((c: string) =>
