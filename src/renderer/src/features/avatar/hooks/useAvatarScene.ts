...
import {
  loadAvatarObject,
  disposeAvatarObject,
} from "@renderer/components/Avatar/avatar3DUtils";
...
export const useAvatarScene = ({
  mountRef,
  userId,
  cookie,
}: UseAvatarSceneOptions) => {
  ...
  const renderAvatar = useCallback(
    async (userId: string) => {
      if (!sceneRef.current || !rendererRef.current || isRenderingRef.current)
        return;
      if (!cookie) {
      ...
      try {
        ...
        const object = await loadAvatarObject({
          userId,
          cookie,
          objectName: "current_avatar",
        });
