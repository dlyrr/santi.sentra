...
export const extractFireInstances = (
  ...
  for (const child of root.children || []) {
    const childFires = extractFireInstances(
      child,
      currentPosition,
      currentSize,
    );
...
const parseFireInstance = (
  ...
  let size = 5;
  const sizeVal =
    props["size_xml"]?.value || props["Size"]?.value || props["size"]?.value;
...
export const extractSparklesInstances = (
  ...
  for (const child of root.children || []) {
    const childSparkles = extractSparklesInstances(
      child,
      currentPosition,
      currentSize,
    );
...
const parseSparklesInstance = (
  ...
  console.log("[Sparkles] Created sparkles instance:", {
    enabled,
    sparkleColor,
    timeScale,
  });
