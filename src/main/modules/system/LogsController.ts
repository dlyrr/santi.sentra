...
const getWindowsLogsPath = (): string => {
  ...
  console.log(
    "[LogsController] LOCALAPPDATA has spaces:",
  ...
  console.log(
    "[LogsController] USERPROFILE has spaces:",
  ...
  if (userProfile) {
    ...
    console.log(
      "[LogsController] Trying fallback path (lowercase logs):",
      fallbackPath,
    );
    ...
    console.log(
      "[LogsController] Trying fallback path (uppercase Logs):",
      fallbackPath,
    );
  ...
  console.error(
    "[LogsController] Neither LOCALAPPDATA nor USERPROFILE are set!",
  );
  return "";
};
...
export const registerLogsHandlers = () => {
  handle("get-logs", z.tuple([]), async () => {
    try {
      console.log(
        "[LogsController] get-logs called, checking LOGS_DIR:",
        LOGS_DIR,
      );
      if (!existsSync(LOGS_DIR)) {
        console.warn(
          "[LogsController] Roblox logs directory does NOT exist at:",
          LOGS_DIR,
        );
        ...
        console.log(
          "  logs dir exists:",
          existsSync(logsDirParent),
          "→",
          logsDirParent,
        );
        console.log(
          "[LogsController] Please note: Roblox must be launched at least once to generate logs",
        );
        return [];
      }
      ...
      try {
        ...
        console.log(
          "[LogsController] Successfully read directory. Found entries:",
          entries.length,
        );
      ...
      for (const file of files) {
        try {
          ...
          try {
            ...
            console.warn(
              `[LogsController] Could not read file ${file}:`,
              readErr,
            );
            continue;
          }
          ...
          const channelMatch = content.match(
            /\[FLog::ClientRunInfo\] The channel is (\w+)/,
          );
          ...
          const versionMatchC = content.match(
            /userAgent: Roblox\/[^/]+\/([\d.]+)/,
          );
  ...
  handle("delete-all-logs", z.tuple([]), async () => {
    try {
      ...
      await Promise.all(
        logFiles.map((f) =>
          fs
            .unlink(path.join(LOGS_DIR, f))
            .catch((e) => console.error(`Failed to delete ${f}:`, e)),
        ),
      );
      return true;
    } catch (error) {
    ...
    try {
      ...
      if (process.platform === "win32") {
        try {
          ...
          console.error(
            "Failed to launch Notepad, falling back to default handler:",
            err,
          );
        }
      }
