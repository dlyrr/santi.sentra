import { useQuery, useInfiniteQuery, useQueries, useSuspenseQueries } from '@tanstack/react-query'
...
export const useTransactionTypes = (cookie?: string) => {
  return useQuery({
    ...
    queryFn: () => transactionsQueue.enqueue(() => window.api.getTransactionTypes(cookie!)),
    enabled: !!cookie,
...
export const useTransactions = (
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      return transactionsQueue.enqueue(() => window.api.getTransactions(cookie!, transactionType, pageParam, 100))
    },
...
export const useTransactionTotals = (
  ...
  return useQuery({
    ...
    queryFn: () => transactionsQueue.enqueue(() => window.api.getTransactionTotals(cookie!, timeFrame)),
    enabled: !!cookie,
    staleTime: 2 * 60 * 1000 // 2 minutes
  })
}
...
export const useBatchTransactions = (
  ...
  return useQueries({
    queries: cookies.map((cookie) => ({
      queryKey: queryKeys.transactions.list(cookie, transactionType),
      queryFn: () => transactionsQueue.enqueue(() => window.api.getTransactions(cookie, transactionType, undefined, 100)),
      enabled: !!cookie && enabled,
      staleTime: 2 * 60 * 1000
    })),
    combine: (results) => {
...
export const useBatchTransactionTypes = (
  cookies: string[]
) => {
  return useQueries({
    queries: cookies.map((cookie) => ({
