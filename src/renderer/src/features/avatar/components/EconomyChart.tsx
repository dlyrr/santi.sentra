...
export const EconomyChart: React.FC<EconomyChartProps> = ({
  ...
  const handleZoomIn = useCallback(() => {
    if (chartRef.current) {
      ...
      if (currentRange) {
        const center =
          ((currentRange.from as number) + (currentRange.to as number)) / 2;
        const newHalfRange =
          ((currentRange.to as number) - (currentRange.from as number)) / 4;
    ...
    if (chartRef.current) {
      ...
      if (currentRange) {
        const center =
          ((currentRange.from as number) + (currentRange.to as number)) / 2;
        const newHalfRange =
          (currentRange.to as number) - (currentRange.from as number);
        timeScale.setVisibleRange({
          from: (center - newHalfRange) as Time,
          to: (center + newHalfRange) as Time,
        });
  ...
  useEffect(() => {
    ...
    const chartData: AreaData<Time>[] = filteredData.map((p) => ({
      time: p.time,
      value: p.value,
    }));
    ...
    if (showMA && maData.length > 0) {
      ...
      const maChartData: LineData<Time>[] = maData.map((p) => ({
        time: p.time,
        value: p.value,
      }));
...
export const CombinedChart: React.FC<CombinedChartProps> = ({
  ...
  const valueData = useMemo(
    () => parseValueChanges(valueChanges),
    [valueChanges],
  );
  const rapData = useMemo(
    () => parseRapHistory(historyData || null),
    [historyData],
  );
  ...
  if (valueData.length === 0 && rapData.length === 0) {
    return (
      <EmptyStateCompact message="No chart data available" className="p-8" />
    );
  }
