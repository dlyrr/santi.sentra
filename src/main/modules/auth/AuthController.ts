...
export const registerAuthHandlers = (): void => {
  handle("validate-cookie", z.tuple([z.string()]), async (_, cookieRaw) => {
    ...
    try {
      const detailed = await RobloxUserService.getDetailedStats(
        cookie,
        userData.id,
      );
      ...
      if (birthdate) {
        ...
        if (
          ...
          (today.getMonth() + 1 === birthdate.birthMonth &&
            today.getDate() < birthdate.birthDay)
        ) {
          age--;
        }
      }
      ...
      console.warn(
        "Failed to fetch detailed stats or birthdate during validation",
        e,
      );
      return userData;
    }
  ...
  handle(
    "open-browser-with-account",
    z.tuple([z.string(), z.string().optional()]),
    async (_, accountId, url) => {
