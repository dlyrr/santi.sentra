...
export const EnhancedBadge: React.FC<EnhancedBadgeProps> = ({
  ...
  const variantClasses = {
    ...
    error: 'bg-red-500/15 text-red-300 border border-red-500/30 ring-red-500/20',
    warning:
...
export const EnhancedProgressBar: React.FC<EnhancedProgressBarProps> = ({
  ...
  return (
    <div className={cn('w-full rounded-full bg-[var(--color-surface-hover)]/50 overflow-hidden', className)}>
      <motion.div
        initial={{ width: 0 }}
  ...
  <>
    {Array.from({ length: count }).map((_, i) => (
      <motion.div
        ...
        className={cn('rounded-lg bg-[var(--color-surface-hover)]/50', className)}
      />
    ))}
  </>
)
