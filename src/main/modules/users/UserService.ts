...
export class RobloxUserService {
  private static appendQueryParam(
    url: string,
    key: string,
    value: string,
  ): string {
  ...
  static async getAvatarUrl(userId: string | number): Promise<string> {
    ...
    const map = await RobloxUserService.getBatchUserAvatarHeadshots(
      [numeric],
      "420x420",
    );
  ...
  static async getBatchUserAvatarHeadshots(
    ...
    const fetchChunk = async (chunkIds: number[]): Promise<void> => {
      ...
      try {
          ...
          response.data.forEach((entry) => {
            if (entry.state === "Completed" && entry.imageUrl) {
              const cacheBusted = entry.version
                ? RobloxUserService.appendQueryParam(
                    entry.imageUrl,
                    "v",
                    entry.version,
                  )
        ...
        console.error(
          "[RobloxUserService] Failed to fetch batch user avatars for chunk:",
          error,
        );
    ...
    if (userIds.length > 0 && firstValidCookie) {
      try {
        ...
        for (let i = 0; i < userIds.length; i += chunkSize) {
          ...
          const presenceResult = await requestWithCsrf(
            userPresenceResponseSchema,
            {
              method: "POST",
              url: "https://presence.roblox.com/v1/presence/users",
  ...
  static async getBatchPresences(cookie: string, userIds: number[]) {
    ...
    const chunks = Array.from(
      { length: Math.ceil(userIds.length / chunkSize) },
  ...
  static async getExtendedUserDetails(cookie: string, userId: number) {
    ...
    const isPremium = (() => {
      ...
      if (premiumData && typeof premiumData === "object") {
        ...
        if (typeof payload.isEligibleForPremiumFeatures === "boolean")
          return payload.isEligibleForPremiumFeatures;
    ...
    const avatarImageUrl =
      avatarThumbnail.data && avatarThumbnail.data[0]
        ? avatarThumbnail.data[0].imageUrl
        : null;
  ...
  static async getPastUsernames(_cookie: string, userId: number) {
    try {
      ...
      if (e.statusCode === 429) {
        console.warn(
          `Rate limit hit for past usernames (user ${userId}). Returning empty list.`,
        );
  ...
  static async getBatchJoinDates(
    ...
    const fetchChunk = async (chunkIds: number[]): Promise<void> => {
      try {
        ...
        console.error(
          "[RobloxUserService] Failed to fetch batch join dates:",
          error,
        );
        chunkIds.forEach((id) => resultMap.set(id, null));
      }
  ...
  static async getBatchUserDetails(
    ...
    Map<number, { id: number; name: string; displayName: string } | null>
  > {
    ...
    const fetchChunk = async (chunkIds: number[]): Promise<void> => {
      try {
        ...
        console.error(
          "[RobloxUserService] Failed to fetch batch user details for chunk:",
          error,
        );
        chunkIds.forEach((id) => resultMap.set(id, null));
      }
