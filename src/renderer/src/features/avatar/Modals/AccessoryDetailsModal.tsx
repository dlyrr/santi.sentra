...
const AccessoryDetailsModal: React.FC<AccessoryDetailsModalProps> = ({
  ...
  const isLimited =
    details?.isLimited || details?.isLimitedUnique || !!rolimonsItem;
  const { data: rolimonsPageData, isLoading: rolimonsPageLoading } =
    useRolimonsItemPage(
      currentAssetId,
  ...
  const handleDownloadObj = async (assetId: number, assetName: string) => {
    try {
      ...
      if (!result?.success) {
        showNotification("Failed to download OBJ", "error");
        console.error("Failed to download OBJ");
      }
    } catch (err: any) {
      showNotification(`Failed to download OBJ: ${err.message || String(err)}`, "error");
  ...
  const handleDownloadTexture = async (assetId: number, assetName: string) => {
    try {
      ...
      if (!result?.success) {
        showNotification("Failed to download texture", "error");
        console.error("Failed to download texture");
      }
    } catch (err: any) {
      showNotification(`Failed to download texture: ${err.message || String(err)}`, "error");
  ...
  const handleDownloadTemplate = async (assetId: number, assetName: string) => {
    try {
      ...
      if (!result.success) {
        showNotification(`Failed to download template: ${result.message}`, "error");
        console.error("Failed to download template:", result.message);
      }
    } catch (err: any) {
      showNotification(`Failed to download template: ${err.message || String(err)}`, "error");
          ...
          {isLoading && !details ? (
              ...
              <div className="flex flex-col lg:flex-row flex-1 min-h-0">
                ...
                <div className="w-full lg:w-1/2 flex flex-col overflow-hidden bg-[var(--color-app-bg)]">
                  <Tabs
                    ...
                    activeTab={activeTab}
                    onTabChange={(tabId) =>
                      setActiveTab(tabId as "info" | "economy")
                    }
            ...
            <div className="p-8 flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-[var(--color-text-secondary)] mb-4">
                Unable to fetch asset details. You may need to be logged in or
                have a valid Roblox cookie to view purchase information.
              </p>
            </div>
          )}
