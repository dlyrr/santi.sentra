...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogClose, DialogBody } from '../../../components/UI/dialogs/Dialog'
      ...
      <DialogContent className="max-w-md">
        ...
        <DialogBody className="px-4 py-3 space-y-4">
          <div className="flex items-center gap-3">
            ...
            >
              <RotateCcw size={14} className={isLoadingUserAgent ? "animate-spin" : ""} />
              Rotate to Next
            </button>
            <div className="text-xs text-[var(--color-text-muted)]">
              Current: <span className="font-semibold text-[var(--color-text-primary)]">#{userAgentIndex + 1}</span> of {allUserAgents.length}
            </div>
          </div>

          <div className="bg-[var(--color-surface-hover)] p-3 rounded-[var(--control-radius)] border border-[var(--color-border)]">
            <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-muted)] mb-1">Active User Agent</p>
            <p className="text-xs text-[var(--color-text-primary)] break-words font-mono leading-relaxed">{currentUserAgent || 'Loading...'}</p>
          </div>

          <div className="space-y-2">
            <label className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-muted)]">Select from {allUserAgents.length} Agents</label>
            <div className="h-48 overflow-y-auto border border-[var(--color-border)] rounded-[var(--control-radius)] bg-[var(--color-surface-muted)] styled-scrollbar divide-y divide-[var(--color-border)]/50">
              {allUserAgents.map((ua, index) => {
                ...
                return (
                  ...
                  >
                    <span className={`text-[10px] font-bold mt-0.5 shrink-0 w-6 ${isActive ? 'text-[var(--accent-color)]' : 'text-[var(--color-text-muted)] group-hover:text-[var(--color-text-primary)]'}`}>
                      #{index + 1}
                    </span>
          ...
          <div className="flex items-center gap-4 p-3.5 bg-[var(--color-surface-hover)] rounded-[var(--control-radius)] border border-[var(--color-border)]">
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-primary)]">Auto-rotate User Agents</label>
                <span className="text-xs font-medium text-[var(--accent-color)]">{autoSwapInterval} min</span>
              </div>
              <input
