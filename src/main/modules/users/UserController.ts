...
export const registerUserHandlers = (): void => {
    ...
    async (_, cookieRaw, newDisplayName) => {
      try {
        ...
        if (!authData || !authData.id) {
          throw new Error(
            "Could not resolve authenticated user identity from cookie session.",
          );
        }

        return await AccountSettingsService.updateDisplayName(
          cookie,
          authData.id,
          newDisplayName,
        );
        ...
        console.error(
          "[UserController] Failed live platform synchronization:",
          err,
        );
        return {
          success: false,
  ...
  handle(
    ...
    async (_, cookieRaws) => {
      ...
      const results =
        await RobloxUserService.getBatchAccountStatuses(extractedCookies);

      const resultObj: Record<
        string,
        { userId: number; presence: any } | null
      > = {};
  ...
  handle(
    ...
    async (_, username) => {
      try {
        ...
        console.error(
          "[UserController] Failed to get avatar URL for username:",
          username,
          err,
        );
        return { success: false, url: "" };
