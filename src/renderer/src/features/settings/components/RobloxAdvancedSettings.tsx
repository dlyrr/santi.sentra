  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2 z-10 relative">
        <div
          className={`w-7 h-7 rounded-[calc(var(--control-radius)-4px)] flex items-center justify-center transition-colors ${disabled ? "bg-[var(--color-surface-secondary)] text-[var(--color-text-muted)]" : "bg-[var(--color-surface-hover)] text-[var(--color-text-secondary)] group-hover:text-[var(--accent-color)] border border-[var(--color-border)]"}`}
        >
          {icon}
        </div>
      ...
      <div className="mt-auto z-10 relative">
        <h4 className="text-xs font-bold text-[var(--color-text-primary)] leading-tight mb-0.5">
          {label}
        </h4>
        <p className="text-[9px] text-[var(--color-text-muted)] leading-snug line-clamp-2">
          {desc}
        </p>
      </div>
...
export default function RobloxAdvancedSettings({
  ...
  const set = useCallback(
    <K extends keyof RobloxSettings>(key: K, value: RobloxSettings[K]) => {
      ...
      <div className="flex-1 overflow-y-auto px-1 pb-4 styled-scrollbar min-h-0">
        <div className="grid grid-cols-2 gap-2.5">
          ...
          <BentoCard
            ...
            control={
              <CustomCheckbox
                ...
                onChange={() =>
                  set("enableOptimizations", !local.enableOptimizations)
                }
                disabled={busy || !isWindows}
              />
            }
          ...
          <BentoCard
            ...
            control={
              <CustomCheckbox
                checked={local.memoryLimit > 0 || !!local.optimizeRamEnabled}
                onChange={() => {
                  const isNowEnabled = !(
                    local.memoryLimit > 0 || !!local.optimizeRamEnabled
                  );
                  set("optimizeRamEnabled", isNowEnabled);
                  set(
                    "memoryLimit",
                    isNowEnabled ? local.ramOptimizeLimit || 1024 : 0,
                  );
                }}
                disabled={busy || !isWindows}
              />
              ...
              local.framerateCapEnabled && (
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      set("framerateCapValue", parseInt(e.target.value) || 60)
                    }
                    disabled={busy}
                    className="w-16 text-xs px-2 py-1 rounded bg-[var(--color-surface-secondary)] border border-[var(--color-border)] text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--accent-color)] text-center transition-colors"
                  />
              ...
              <select
                ...
                onChange={(e) =>
                  set(
                    ...
                    e.target.value as "Terrain" | "Legacy",
                  )
                }
                disabled={busy}
          ...
          <BentoCard
            ...
            control={
              <CustomCheckbox
                checked={local.disableDualChannelAudio}
                onChange={() =>
                  set("disableDualChannelAudio", !local.disableDualChannelAudio)
                }
                disabled={busy}
              />
            }
