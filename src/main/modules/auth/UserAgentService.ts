...
export class UserAgentService {
  ...
  static setUserAgentIndex(index: number): string {
    const validIndex = Math.max(
      0,
      Math.min(index, this.USER_AGENTS.length - 1),
    );
    const settings = storageService.getSettings();
    ...
    const updatedSettings = {
      ...
      userAgentSettings: {
        ...
        autoSwapUserAgent:
          settings?.userAgentSettings?.autoSwapUserAgent ?? false,
        autoSwapIntervalMinutes:
          settings?.userAgentSettings?.autoSwapIntervalMinutes ?? 30,
      },
    };
  ...
  static startAutoSwap(
    ...
    const updatedSettings = {
      ...
      userAgentSettings: {
        currentUserAgentIndex:
          settings?.userAgentSettings?.currentUserAgentIndex ?? 0,
    ...
    this.autoSwapInterval = setInterval(() => {
      ...
      console.log(
        `[UserAgentService] Auto-swapped user agent to index ${this.getCurrentUserAgentIndex()}`,
      );
      onSwap?.(newUA);
    }, intervalMs);

    console.log(
      `[UserAgentService] Auto-swap started with ${intervalMinutes} minute interval`,
    );
  }
    ...
    if (this.autoSwapInterval) {
      ...
      const updatedSettings = {
        ...
        userAgentSettings: {
          ...
          autoSwapUserAgent: false,
          autoSwapIntervalMinutes:
            settings?.userAgentSettings?.autoSwapIntervalMinutes ?? 60,
        },
      };
      storageService.setSettings(updatedSettings);
