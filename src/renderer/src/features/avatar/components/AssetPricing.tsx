...
import { ShoppingCart, Check, Loader2, CheckCircle2, AlertTriangle, XCircle } from 'lucide-react'
...
import { catalogPurchaseLimiter, executeWithRetry, isRateLimitError, sleep } from '@renderer/lib/rateLimiter'
...
const PurchaseConfirmDialog: React.FC<{
  ...
  accountCount?: number
}> = ({ isOpen, onClose, onConfirm, assetName, price, userBalance, accountCount = 1 }) => {
    ...
    <Dialog isOpen={isOpen} onClose={onClose}>
      <DialogContent className="max-w-sm">
        <DialogBody className="flex flex-col items-center text-center py-6">
          ...
          <h2 className="text-xl font-bold text-[var(--color-text-primary)] mb-3">Confirm Purchase</h2>
          <p className="text-[var(--color-text-secondary)] text-sm leading-relaxed mb-6">
            Are you sure you want to buy{' '}
            <span className="font-semibold text-[var(--color-text-primary)]">&quot;{assetName}&quot;</span> for{' '}
          ...
          {isBulkPurchase && (
            <div className="w-full bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 mb-6 text-sm text-blue-100">
              Purchasing for {accountCount} selected accounts. Accounts that already own it are
              skipped.
            </div>
          )}
          {hasBalance && typeof price === 'number' && (
            <div className="w-full bg-[var(--color-surface-hover)]/50 rounded-lg p-3 mb-6 text-sm">
              ...
              <div className="flex justify-between items-center pt-2 border-t border-[var(--color-border-strong)]">
                <span className="text-[var(--color-text-secondary)]">After Purchase</span>
                <span
                  className={cn(
  ...
  <Dialog isOpen={isOpen} onClose={onClose}>
    <DialogContent className="max-w-sm">
      <DialogBody className="flex flex-col items-center text-center py-8">
        ...
        <h2 className="text-2xl font-bold text-[var(--color-text-primary)] mb-6">Purchase Complete</h2>
        ...
        <p className="text-[var(--color-text-secondary)] text-base leading-relaxed">
          ...
          <span className="font-semibold text-[var(--color-text-primary)]">{assetName}</span> from{' '}
          <span className="font-semibold text-[var(--color-text-primary)]">{creatorName}</span> for{' '}
          <span className="inline-flex items-center gap-1 font-semibold text-emerald-400">
            {typeof price === 'number' ? price.toLocaleString() : price}
            {typeof price === 'number' && <RobuxIcon className="w-4 h-4" />}
          </span>
  ...
  <Dialog isOpen={isOpen} onClose={onClose}>
    <DialogContent className="max-w-sm">
      <DialogBody className="flex flex-col items-center text-center py-6">
        ...
        <h2 className="text-xl font-bold text-[var(--color-text-primary)] mb-3">Purchase Failed</h2>
        <p className="text-[var(--color-text-secondary)] text-sm leading-relaxed mb-6">{errorMessage}</p>
...
export const AssetPricing: React.FC<AssetPricingProps> = ({
  ...
  const [purchaseProgress, setPurchaseProgress] = useState<{ current: number; total: number; success: number; failed: number } | null>(null)
  ...
  const targetAccounts = useMemo<PurchaseTargetAccount[]>(() => {
    ...
    return cookie && userId ? [{ cookie, userId, id: userId, username: 'You' }] : []
  ...
  React.useEffect(() => {
    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        if (!isBulkMode && !ownedSet.has(targetAccounts[0]?.id) && displayPrice !== 'Off Sale') {
          const stats = await window.api.fetchAccountStats(targetAccounts[0].cookie)
  ...
  const accountsNeedingPurchase = targetAccounts.filter((a) => !ownedAccounts.has(a.id))
  const isFullyOwned = accountsNeedingPurchase.length === 0 && targetAccounts.length > 0
  const totalCost =
    accountsNeedingPurchase.length * (typeof displayPrice === 'number' ? displayPrice : 0)
  ...
  const handleConfirmPurchase = async () => {
    ...
    if (!canPurchase || !details.collectibleItemId || targetAccounts.length === 0) return
    ...
    try {
      ...
      let lastError = ''
      let currentIdx = 0
      const purchaseResults: Array<{ accountId: string; success: boolean; error?: string }> = []
      ...
      if (isBulkMode) {
        setPurchaseProgress({ current: 0, total: accountsNeedingPurchase.length, success: 0, failed: 0 })
        ...
        for (const acc of accountsNeedingPurchase) {
          try {
            ...
            const authCheck = await window.api.validateCookie(acc.cookie).catch(() => null)
            if (!authCheck) {
              lastError = 'Invalid cookie (Unauthenticated)'
              purchaseResults.push({ accountId: acc.id, success: false, error: lastError })
              currentFailed++
            } else {
              ...
              if (result.purchased) {
                ...
                const error = result.errorMessage || result.reason || 'Unknown error'
                lastError = error
                purchaseResults.push({ accountId: acc.id, success: false, error })
                currentFailed++
              }
            }
          } catch (err: any) {
            ...
            lastError = errorMsg
            purchaseResults.push({ accountId: acc.id, success: false, error: errorMsg })
          }
          currentIdx++
          setPurchaseProgress({ current: currentIdx, total: accountsNeedingPurchase.length, success: currentSuccess, failed: currentFailed })
        ...
        if (currentSuccess > 0) {
          onPurchaseSuccess?.(details, isBulkMode ? currentSuccess * expectedPrice : displayPrice)
        } else if (currentFailed > 0) {
          onPurchaseError?.(`Purchase failed for all. Last error: ${lastError}`)
        }
      } else {
        ...
        try {
          const result = await executeWithRetry(
            ...
            async () => {
              return await window.api.purchaseCatalogItem(
                ...
                expectedSellerId,
                details.collectibleProductId || '',
                accountsNeedingPurchase[0].userId || accountsNeedingPurchase[0].id,
                crypto.randomUUID()
              )
            },
            { retryCondition: isRateLimitError }
          ...
          if (result.purchased) {
            setOwnedAccounts((prev) => new Set(prev).add(accountsNeedingPurchase[0].id))
            successCount++
          } else {
  ...
  return (
    <div className="p-4 bg-[var(--color-surface)]/50 border border-[var(--color-border)] rounded-xl flex items-center justify-between relative">
      <div>
        ...
        <div className="text-xl font-bold text-[var(--color-text-primary)] flex items-center gap-1">
          {typeof displayPrice === 'number' ? (
            ...
            <span className={displayPrice === 'Free' ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-text-secondary)]'}>
              {displayPrice}
            </span>
          )}
        </div>
      ...
      <Button
        disabled={
          isFullyOwned || displayPrice === 'Off Sale' || isPurchaseUnavailable || isPurchasing
        }
      ...
      >
        {isPurchasing ? (
          ...
          <span className="flex items-center flex-col py-1">
            <span className="flex items-center">
              <ShoppingCart size={18} className="mr-2" />
              {isBulkMode ? `Buy for All (${accountsNeedingPurchase.length})` : 'Buy'}
            </span>
      ...
      {isPurchasing && purchaseProgress && isBulkMode && (
        <div className="absolute left-0 right-0 -bottom-[100px] bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-4 z-20 shadow-2xl flex flex-col gap-3 min-w-[300px]">
          <div className="flex justify-between items-center">
            <span className="text-sm font-semibold text-[var(--color-text-primary)]">Purchasing...</span>
            <span className="text-xs text-[var(--color-text-secondary)]">{purchaseProgress.current} / {purchaseProgress.total}</span>
          </div>
          
          <div className="w-full bg-[var(--color-surface-hover)] rounded-full h-2 overflow-hidden flex">
            <div 
              className="bg-emerald-500 h-full transition-all duration-300" 
              style={{ width: `${(purchaseProgress.success / purchaseProgress.total) * 100}%` }}
            />
            <div 
              className="bg-red-500 h-full transition-all duration-300" 
              style={{ width: `${(purchaseProgress.failed / purchaseProgress.total) * 100}%` }}
            />
          </div>
